...
const report = (records, live = [], anonymize = false) =>
  renderReport({
    ...
    live: Array.isArray(live) ? { sessions: live, blind: [] } : live,
...
test('sessions predating turn_duration are called out, not silently zeroed', () => {
  ...
  assert.match(html, /per-turn duration/);
});
...
const twoAgents = () => [
  session({ agent: 'claude', sessionId: 'cl', label: 'acme' }),
  session({
    agent: 'codex',
    sessionId: 'cx',
    label: 'api',
    project: '/Users/me/clients/beta',
    cwd: '/Users/me/clients/beta',
    activeMs: HOUR,
    spans: [{ start: T0 + 5 * HOUR, end: T0 + 6 * HOUR }],
    startedAt: T0 + 5 * HOUR,
    endedAt: T0 + 6 * HOUR,
  }),
];

test('a mixed report names both agents in its heading', () => {
  const html = report(twoAgents());
  assert.match(html, /<h1>Claude Code and Codex sessions<\/h1>/);
});

test('a single-agent report does not grow an agent breakdown it cannot use', () => {
  const html = report([session()]);
  assert.ok(!html.includes('Which agents did the work'));
  assert.ok(!html.includes('<span class="tag">'), 'no agent tags when there is one agent');
});

test('a mixed report breaks the work down by agent', () => {
  const html = report(twoAgents());
  assert.match(html, /Which agents did the work/);
  assert.match(html, /<span class="tag">codex<\/span>/);
  assert.match(html, /Concurrency is counted per <strong>session<\/strong>/);
});

test('an agent that cannot report live state says so instead of showing nothing', () => {
  const html = report(twoAgents(), {
    sessions: [],
    blind: [{ id: 'codex', name: 'Codex', note: 'publishes no live session state' }],
  });
  assert.match(html, /Right now/);
  assert.match(html, /Codex — publishes no live session state/);
});

test('the footer names the roots it read, folded to ~', () => {
  const html = renderReport({
    stats: computeStats([session()]),
    live: { sessions: [], blind: [] },
    anon: new Anonymizer(false),
    sources: [{ id: 'claude', name: 'Claude Code', root: `${home}/.claude` }],
    windowLabel: 'Last 30d',
    generatedAt: T0,
  });
  assert.match(html, /Claude Code \(~\/\.claude\)/);
});

test('--anonymize keeps the source paths out of the footer too', () => {
  const html = report([session()], [], true);
  assert.ok(!html.includes('/home/me/.claude'), 'the path must not survive anonymisation');
  assert.match(html, /Generated by agentclock from Claude Code ·/);
});
